...
def match_lcs(b, s1, s2, match_result):
    i = len(s1)
    j = len(s2)
    if i == 0 and j == 0:
        return
...
def get_extended_arm(match_pairs, is_eq_seq):
    idx = 0
    prev_match_idx = -1
...
def seq_to_match_pairs(seq):
    return list(map(lambda c: (MATCH, c), seq))


def match_pairs_to_seq(match_pairs):
...
if __name__ == '__main__':
    ...
    while k_mer_start <= len(full_seq) - HAIRPIN_MIN:
        k_mer = full_seq[k_mer_start: k_mer_start + k_unit]
        ...
        seq_in_distance = full_seq[k_mer_start + LOOP_MAX: k_mer_start + HAIRPIN_MAX]
        k_mer_rev = reverse_seq(k_mer)

        # only first k_mer_rev is used.
        # FIXME k_mer_rev 가 지켜야할 조건은 loop max 조건밖에 없는데, 이 조건은 작은 것에서 지켜지지 않고 큰것에서 지켜지는 경우가 없다.
        k_mer_rev_start = seq_in_distance.find(k_mer_rev)

        if k_mer_rev_start == -1:
            k_mer_start += 1
        else:
            k_mer_rev_start = k_mer_rev_start + LOOP_MAX + k_mer_start
            k_seq_middle = full_seq[k_mer_start + k_unit: k_mer_rev_start]

            # FIXME // 2 를 할 때, 홀짝을 고려해야할 수 있음. 일단은 나이브하게.
            k_seq_left_right_len = (HAIRPIN_MAX - 2 * k_unit - len(k_seq_middle)) // 2
            ...
            k_seq_right = full_seq[k_mer_rev_start + k_unit: k_mer_rev_start + k_unit + k_seq_left_right_len]

            k_seq_right_rev = reverse_seq(k_seq_right)
            k_seq_middle_rev = reverse_seq(k_seq_middle)
